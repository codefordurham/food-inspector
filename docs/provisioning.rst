...
As documented you should create/import your Celery app in ``eatsmart/__init__.py`` so that you
can run the worker via::

    python celery -A eatsmart worker

Additionally you will need to configure the project settings for Celery::

    # eatsmart.settings.staging.py
    import os
...
You will also need to add the ``BROKER_URL`` to the ``eatsmart.settings.production`` so
...
RabbitMQ user will be named eatsmart_<environment> and the vhost will be named eatsmart_<environment>
