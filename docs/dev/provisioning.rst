...
Eatsmart is deployed on the following stack.
...
documented on that page, you need to create a new file in ``eatsmart/celery.py`` and then
modify ``eatsmart/__init__.py`` to import that file. You'll also need to customize ``{{
...
Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::
    ...
    # ...
    BROKER_URL = 'amqp://eatsmart_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/eatsmart_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``eatsmart_<environment>`` and the
vhost will be named ``eatsmart_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out eatsmart.key 2048
  # Make copy of the key with the passphrase
  cp eatsmart.key eatsmart.key.secure
  # Remove any passphrase
  openssl rsa -in eatsmart.secure -out eatsmart.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key eatsmart.key -out eatsmart.csr
