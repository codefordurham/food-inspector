...
class Establishment(models.Model):
    ...
    STATUS_CHOICES = (('deleted', ugettext_lazy('Deleted')),
    ...
    property_id = models.CharField(ugettext_lazy("Property ID"),
                                   max_length=128, blank=True)
    name = models.CharField(ugettext_lazy("Name"), max_length=255)
    pretty_name = models.CharField(ugettext_lazy("Pretty Name"),
                                   max_length=255, blank=True)
    type = models.PositiveIntegerField(ugettext_lazy("Type"),
                                       default=0, choices=TYPE_CHOICES)
    address = models.CharField(ugettext_lazy("Address"), max_length=255)
    city = models.CharField(ugettext_lazy("City"), max_length=64)
    county = models.CharField(ugettext_lazy("County"), max_length=64,
                              db_index=True)
    ...
    update_date = models.DateTimeField(ugettext_lazy("Update Date"),
                                       null=True, blank=True, db_index=True)
    status = models.CharField(ugettext_lazy("Status"), choices=STATUS_CHOICES,
                              max_length=32, default='active')
    location = models.PointField(ugettext_lazy("location"),
...
class Inspector(models.Model):
    ...
    phone_number = models.CharField(ugettext_lazy("Phone Number"),
                                    max_length=64, blank=True)
    e_mail = models.CharField(ugettext_lazy("E-mail address"), max_length=128,
                              blank=True)
    county = models.CharField(ugettext_lazy("County"), max_length=64,
                              db_index=True)
...
class Inspection(models.Model):
    ...
    update_date = models.DateTimeField(ugettext_lazy("Update Date"), null=True,
...
class Violation(models.Model):
    ...
    inspection = models.ForeignKey(Inspection, related_name='violations',
                                   verbose_name=ugettext_lazy("Inspection"),
                                   null=True, blank=True)
    external_id = models.CharField(ugettext_lazy("External ID"),
                                   max_length=128)
    date = models.DateTimeField(ugettext_lazy("Date"), db_index=True)
